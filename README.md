Alibaba Cloud RocketMQ Data Consistency Terraform Module

[简体中文](README-CN.md) | English

# terraform-alicloud-rocketmq-data-consistency

This Terraform module implements a [distributed transaction solution using RocketMQ](https://www.aliyun.com/solution/middleware/rocketmq) transactional messages on Alibaba Cloud. It creates a complete infrastructure including VPC, ECS instances, RDS database, and RocketMQ resources to demonstrate distributed data consistency.

## Features

- **VPC and Network Infrastructure**: Creates VPC, vSwitches, and security groups
- **ECS Instances**: Provisions provider and consumer instances with automated setup scripts
- **RDS MySQL Database**: Sets up MySQL database with proper accounts and permissions
- **RocketMQ Integration**: Configures RocketMQ instance with topics, consumer groups, and ACL rules
- **Automated Deployment**: Uses ECS commands to automatically configure application environment
- **Security**: Implements proper security group rules and RAM user management
...
│                            VPC                                  │
│  ┌─────────────────────┐    ┌─────────────────────────────────┐ │
│  │   ECS vSwitch       │    │         RDS vSwitch             │ │
│  │                     │    │                                 │ │
│  │ ┌─────────────────┐ │    │ ┌─────────────────────────────┐ │ │
│  │ │ ECS Provider    │ │    │ │        RDS MySQL            │ │ │
│  │ │ Instance        │ │    │ │        Instance             │ │ │
│  │ └─────────────────┘ │    │ └─────────────────────────────┘ │ │
│  │                     │    │                                 │ │
│  │ ┌─────────────────┐ │    └─────────────────────────────────┘ │
...
- [complete](https://github.com/alibabacloud-automation/terraform-alicloud-rocketmq-data-consistency/tree/main/examples/complete) - Creates a full RocketMQ data consistency solution
...
1. **Password Security**: All passwords are marked as sensitive and should be stored securely
2. **Network Security**: Security groups are configured with minimal required access
3. **Database Security**: RDS instance is deployed in a private subnet with restricted access
4. **RAM Security**: RAM user is created with minimal required permissions

## Cost Considerations

This module creates several billable resources:

- ECS instances (2x)
- RDS MySQL instance
- RocketMQ instance
- VPC NAT Gateway (if internet access is needed)
- Data transfer costs

Estimated monthly cost: $100-300 USD (varies by region and instance types)

## Troubleshooting

### Common Issues

1. **Instance Type Availability**: Ensure the specified instance types are available in your region
2. **Password Requirements**: Passwords must meet complexity requirements
3. **Resource Limits**: Check your account limits for ECS, RDS, and RocketMQ instances
4. **Network Configuration**: Ensure CIDR blocks don't overlap with existing networks

### Debugging

1. Check ECS command execution logs in the Alibaba Cloud console
2. Verify RocketMQ instance status and network configuration
3. Test database connectivity from ECS instances
4. Monitor application logs for transaction processing

## Submit Issues

If you have any problems when using this module, please open a [GitHub Issue](https://github.com/alibabacloud-automation/terraform-alicloud-rocketmq-data-consistency/issues).
...
MIT License. See [LICENSE](https://github.com/alibabacloud-automation/terraform-alicloud-rocketmq-data-consistency/blob/main/LICENSE) for full details.

## Reference

- [Terraform AliCloud Provider Documentation](https://registry.terraform.io/providers/aliyun/alicloud/latest/docs)
- [What is RocketMQ](https://www.alibabacloud.com/help/en/rocketmq/)
